...
mod tests {
    ...
    fn parse_var_decl_and_assignment() {
        ...
        assert!(matches!(
            ...
            Stmt::VarDecl(name, Expr::IntLit(1), VarKind::Let) if name == "x"
        ));
    ...
    fn parse_if_else() {
        ...
        let Stmt::If(condition, then_body, Some(else_body)) = &program.statements[0] else {
            panic!("expected if/else statement")
        };

        assert_eq!(*condition, Expr::BoolLit(false));
        assert_eq!(then_body.len(), 1);
        assert!(matches!(
            &then_body[0],
            Stmt::ExprStmt(Expr::Call(_, _))
        ));
        assert_eq!(else_body.len(), 1);
        assert!(matches!(&else_body[0], Stmt::ExprStmt(Expr::Call(_, _))));
    }
    ...
    fn parse_else_if() {
        ...
        assert!(matches!(program.statements[0], Stmt::If(_, _, Some(_))));

        let Stmt::If(_, _, Some(else_body)) = &program.statements[0] else {
            panic!("expected if with else branch")
        };

        assert_eq!(else_body.len(), 1);
        assert!(matches!(&else_body[0], Stmt::If(_, _, Some(_))));
    }
    ...
    fn parse_break_and_continue_statements() {
        ...
        let Stmt::While(_, body) = &program.statements[0] else {
            panic!("expected while statement")
        };

        assert_eq!(body.len(), 2);
        assert!(matches!(body[0], Stmt::If(_, _, None)));
    ...
    fn parse_function_expression() {
        ...
        assert!(matches!(
            &program.statements[0],
            Stmt::VarDecl(_, Expr::FunctionExpr(_, _), VarKind::Let)
        ));
    }
    ...
    fn parse_array_literal() {
        ...
        assert!(matches!(
            &program.statements[0],
            Stmt::VarDecl(_, Expr::Array(items), VarKind::Let) if items.len() == 3
        ));
    }
    ...
    fn parse_js_style_operator_aliases() {
        ...
        assert!(matches!(&program.statements[0], Stmt::VarDecl(_, Expr::Binary(_, BinOp::EqEqEq, _), VarKind::Let)));
        assert!(matches!(&program.statements[1], Stmt::VarDecl(_, Expr::Binary(_, BinOp::NeqEq, _), VarKind::Let)));
        assert!(matches!(&program.statements[2], Stmt::VarDecl(_, Expr::Binary(_, BinOp::And, _), VarKind::Let)));
    }
}
